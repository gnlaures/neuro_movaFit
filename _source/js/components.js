...
if ($('.c-addRemoveInput').length) {
    var addRemoveElements = {
        add: $('.c-addRemoveInput__add'),
        rem: $('.c-addRemoveInput__rem'),
        range: 1,
        max: $('.c-addRemoveInput input').attr('max'),
        min: $('.c-addRemoveInput input').attr('min'),
        valueUn: 59.90,
        input: $('.c-addRemoveInput input'),
        productFinalValue: $('.js-finalValue'),
    };

    addRemoveElements.add.on('click', function() {
        var actualValue = parseInt(addRemoveElements.input.val());
        if (actualValue < addRemoveElements.max) {
           addRemoveElements.input
               .val(actualValue + addRemoveElements.range);
           var newPrice = addRemoveElements.valueUn * (actualValue + 1);
           addRemoveElements.productFinalValue
               .text(newPrice.toLocaleString('pt-br', {minimumFractionDigits: 2}));
        } else {
            alert('Você pode comprar no máximo ' + addRemoveElements.max + ' unidades');
        }
    });

    addRemoveElements.rem.on('click', function() {
        var actualValue = parseInt(addRemoveElements.input.val());
        if (actualValue > addRemoveElements.min) {
            addRemoveElements.input
                .val(actualValue - addRemoveElements.range);
            var newPrice = addRemoveElements.valueUn * (actualValue - 1);
            addRemoveElements.productFinalValue
    ...
    $(window).on('load', function() {
        addRemoveElements.input.val(1);
        addRemoveElements.productFinalValue
            .text(addRemoveElements.valueUn
